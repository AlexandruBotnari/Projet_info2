...
#define LIGNE_H      "─"   // U+2500
#define LIGNE_V      "│"   // U+2502
#define COIN_HG      "┌"   // U+250C
#define COIN_HD      "┐"   // U+2510
#define COIN_BG      "└"   // U+2514
#define COIN_BD      "┘"   // U+2518
#define T_GAU        "├"   // U+251C
#define T_DRO        "┤"   // U+2524
#define T_HAU        "┬"   // U+252C
#define T_BAS        "┴"   // U+2534
...
void afficher_prochain(int idxEq, int idxCbt, const Combat *c) {
    ...
    int e    = code / 10, i = code % 10;
    if (e == idxEq && i == idxCbt) printf("➤");
    else putchar(' ');
}
...
void afficher_effets(const Combattant *cbt) {
    for (int e = 0; e < cbt->nb_effets; e++) {
        printf("%s%s%s", COULEUR(CYAN), cbt->effets[e].symbole, COULEUR(RESET));
    }
}
...
void afficher_ligne_combattant(const Combattant *cbt,
    ...
    printf("%s %s%-10s%s |%d| ",
           LIGNE_V, COULEUR(GRAS), cbt->nom, COULEUR(RESET), idxCbt + 1);
    ...
    int occupe = 1 + 10 + 4 + (cbt->nb_effets * 4)
                 + 1 + LARGEUR_BARRE + 1 + 1;
    for (int s = occupe; s < LARGEUR - 2; s++) putchar(' ');
    putchar('\n');
}
